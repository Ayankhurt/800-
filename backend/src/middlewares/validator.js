...
// Error handler middleware - MUST be defined first
...
// Dispute message validation - supports both 'text' and 'message' fields
export const validateDisputeMessage = [
  body('text').optional().trim().notEmpty().withMessage('text cannot be empty if provided'),
  body('message').optional().trim().notEmpty().withMessage('message cannot be empty if provided'),
  body('attachments').optional().isArray().withMessage('attachments must be an array'),
  // At least one of text or message must be provided
  body().custom((value) => {
    if (!value.text && !value.message) {
      throw new Error('Either text or message field is required');
    }
    return true;
  }),
  handleValidationErrors,
...
// Payout approval/rejection validation
export const validatePayoutRejection = [
  body('reason').optional().trim().isLength({ min: 1, max: 500 }).withMessage('reason must be between 1 and 500 characters'),
  handleValidationErrors,
];

// Pagination with offset validation
export const validatePaginationOffset = [
  query('limit')
    .optional()
    .isInt({ min: 1, max: 100 })
    .withMessage('Limit must be between 1 and 100'),
